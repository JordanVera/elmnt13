export function ChampagneToast() {
  return (
    <section className="relative flex min-h-dvh items-center justify-center overflow-hidden bg-blush px-6">
      <div className="pointer-events-none absolute inset-0 flex items-end justify-center gap-2 pb-[18vh] md:gap-6">
      ...
      <h1 className="relative z-10 text-center">
        <span className="block font-serif text-2xl italic text-gold md:text-4xl">
          To your
        </span>
        <span className="font-display mt-2 block text-6xl tracking-tight uppercase md:text-8xl">
          Love.
        </span>
      </h1>
    </section>
...
function Flute({ mirror }: { mirror: boolean }) {
  return (
    ...
    >
      <defs>
        <linearGradient id={`liquid-${mirror ? "r" : "l"}`} x1="0" x2="0" y1="1" y2="0">
          <stop offset="0%" stopColor="#e8c98a" />
          <stop offset="100%" stopColor="#f6e7c2" />
        </linearGradient>
        <linearGradient id={`glass-${mirror ? "r" : "l"}`} x1="0" x2="1" y1="0" y2="0">
      ...
      <path
        ...
        opacity="0.9"
      />
      <ellipse cx="60" cy="18" rx="22" ry="6" fill="none" stroke="#c4a574" strokeWidth="1.6" />
      <text
        x="60"
        y="118"
        textAnchor="middle"
